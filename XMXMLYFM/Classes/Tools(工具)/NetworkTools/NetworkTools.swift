...
class NetworkTools {
    ...
    class func request(_ type : MethodType = .post, url : String, params : [String : Any]?,success : @escaping (_ data : Data)->(), failure : ((Int?, String) ->Void)?) {
        ...
        // 2.发送网络请求
//        ProgressHUD.show()
//        AF.request(url, method: method, parameters: params, encoding: nil, headers: nil, interceptor: <#T##RequestInterceptor?#>, requestModifier: <#T##Session.RequestModifier?##Session.RequestModifier?##(inout URLRequest) throws -> Void#>)
